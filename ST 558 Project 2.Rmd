...
- `year` requests data from a particular year if the user desires data other than the latest available
```{r}
admissions_info <- function(key="D3KHf387z9W0EaDoVZNsvD6aOSHPWZmwDvKpTxpr", ownership="private", rate="desc", n=50, year="latest")
{
...
financial_info <- function(key="D3KHf387z9W0EaDoVZNsvD6aOSHPWZmwDvKpTxpr", state="NC", size="desc", n=50, year="latest")
{
  ...
  financial_data <- 
    parsed_data$results %>%
    as_tibble() %>%
    select("name"=ends_with("name"), "ownership"=ends_with("ownership"), "city"=ends_with("city"), "size"=ends_with("size"), "poverty_rate"=ends_with("poverty_rate"), "median_family_income"=ends_with("income"), "female_fedian_febt"=ends_with("female_students"), "male_median_debt"=ends_with("debt.male_students"))
...
exploration <- function(general_data, cost_data, admissions_data, demographic_data, financial_data, earnings_data) {
...
table(admissions_data$Ownership, admissions_data$Test_Requirements)
...
plot8 <- ggplot(costs_data, aes(x=Ownership, y=Out_of_State_Tuition)) + 
  geom_boxplot(fill = "#112446") + 
  scale_y_continuous(trans="log10") + 
  geom_jitter() +
...
main <- function() {
  ...
  exploration(general_data, cost_data, admissions_data, demographic_data, financial_data, earnings_data)
}
```
